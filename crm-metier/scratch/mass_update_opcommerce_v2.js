...
async function updateOpco() {
  console.log("🚀 Mise à jour optimisée OPCOMMERCE...");
  ...
  for (const code of OPCOMMERCE_NAF_CODES) {
    ...
    // Liste des conditions à tester séparément pour éviter les timeouts
    const conditions = [
    ...
    for (const cond of conditions) {
      ...
      while (true) {
        ...
        if (error) {
          // Si on a un timeout même sur un EQ, c'est que la table est vraiment massive ou pas indexée
          console.error(`❌ Erreur ${cond.field}=${cond.value}:`, error.message);
          break;
        }
        
        if (!data || data.length === 0) break;
        
        updatedForCond += data.length;
        grandTotalUpdated += data.length;
      }
      if (updatedForCond > 0) console.log(`✅ ${cond.field}=${cond.value}: ${updatedForCond} mis à jour`);
    }
  }

  console.log(`\n🎉 TOTAL GÉNÉRAL: ${grandTotalUpdated}`);
}

...
async function diagnoseOpco() {
  console.log("🔍 Analyse des données OPCOMMERCE...");

  // 1. Compter le total avec nom_opco = OPCOMMERCE
  const { count: currentOpcommerce, error: err1 } = await supabase
    .from('crm_leads')
    .select('*', { count: 'exact', head: true })
    .eq('nom_opco', 'OPCOMMERCE');
  
  if (err1) console.error("Erreur total:", err1);
  console.log(`✅ Actuellement marqués OPCOMMERCE: ${currentOpcommerce}`);

  // 2. Vérifier par petits morceaux (Supabase limit) combien d'autres devraient l'être
  // On va utiliser 'in' pour les codes NAF
  // Mais attention, certains codes NAF peuvent être stockés sans le point ou en minuscule
  const cleanCodes = OPCOMMERCE_NAF_CODES.map(c => c.replace('.', '').toUpperCase());
  
  // On va tester quelques codes un par un pour voir
  let totalMissing = 0;
  
  // Pour éviter de dépasser la limite, on va faire une boucle sur les codes
  for (const code of OPCOMMERCE_NAF_CODES) {
    const cleanCode = code.replace('.', '').toUpperCase();
    const { count, error } = await supabase
      .from('crm_leads')
      .select('*', { count: 'exact', head: true })
      .or(`code_naf.ilike.${code},code_naf.ilike.${cleanCode}`)
    ...
    if (error) {
      ...
      if (count > 0) {
        console.log(`❌ ${code} (${cleanCode}): ${count} manquants`);
        totalMissing += count;
      }
    }
  }

  console.log(`\n📊 TOTAL MANQUANTS ESTIMÉ: ${totalMissing}`);
}

...
async function updateOpco() {
  console.log("🚀 Démarrage de la mise à jour OPCOMMERCE...");
  ...
  for (const code of OPCOMMERCE_NAF_CODES) {
    ...
    let updatedForCode = 0;
    while (true) {
      // On met à jour par paquets de 1000
      ...
      updatedForCode += data.length;
      grandTotalUpdated += data.length;
      process.stdout.write(`✅ ${updatedForCode} mis à jour... `);
    }
    console.log(`\n✨ Terminé pour ${code}: ${updatedForCode} modifiés.`);
  }

  console.log(`\n🎉 TOUT EST FINI ! TOTAL GÉNÉRAL MIS À JOUR: ${grandTotalUpdated}`);
}

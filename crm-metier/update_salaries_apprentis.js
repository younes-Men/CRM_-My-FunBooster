...
async function run() {
  ...
  const apprentisIdx = headers.findIndex(h => h === 'APPRENTIS');

  console.log(`Indices: SIRET=${siretIdx}, NOM=${nomIdx}, SALARIÉS=${salariesIdx}, APPRENTIS=${apprentisIdx}`);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    let apprentis = row[apprentisIdx];

    if (!siret && !nom) continue; // Skip empty
    if (salaries === undefined && apprentis === undefined) continue; // Nothing to update
    ...
    if (apprentis !== undefined && apprentis !== null && apprentis !== '') {
        updates.nb_apprentis = parseInt(apprentis, 10);
    }
    ...
    if (i % 500 === 0) console.log(`Processed ${i}/${data.length} rows...`);
  }

  console.log(`Done! Updated ${updatedCount} records. ${notFoundCount} not found in DB.`);
}
